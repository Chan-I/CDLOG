...
void cdlog_category_del(cdlog_category_t * a_category)
{
	...
	zc_debug("cdlog_category_del[%p]", a_category);
    free(a_category);
	return;
}
 ...
 */
static void cdlog_cateogry_overlap_bitmap(cdlog_category_t * a_category, cdlog_rule_t *a_rule)
{
	int i;
...
static int cdlog_category_obtain_rules(cdlog_category_t * a_category, zc_arraylist_t * rules)
{
	int i;
	int count = 0;
	int fit = 0;
	cdlog_rule_t *a_rule;
	cdlog_rule_t *wastebin_rule = NULL;
	...
	zc_arraylist_foreach(rules, i, a_rule) {
		fit = cdlog_rule_match_category(a_rule, a_category->name);
		if (fit) {
			if (zc_arraylist_add(a_category->fit_rules, a_rule)) {
				zc_error("zc_arrylist_add fail");
				goto err;
			}
			cdlog_cateogry_overlap_bitmap(a_category, a_rule);
			count++;
		}

		if (cdlog_rule_is_wastebin(a_rule)) {
	...
	if (count == 0) {
		if (wastebin_rule) {
			...
			cdlog_cateogry_overlap_bitmap(a_category, wastebin_rule);
			count++;
		} else {
...
cdlog_category_t *cdlog_category_new(const char *name, zc_arraylist_t * rules)
{
	...
	a_category = calloc(1, sizeof(cdlog_category_t));
	if (!a_category) {
	...
	if (cdlog_category_obtain_rules(a_category, rules)) {
		zc_error("cdlog_category_fit_rules fail");
		goto err;
	}

	cdlog_category_profile(a_category, ZC_DEBUG);
	return a_category;
err:
...
int cdlog_category_update_rules(cdlog_category_t * a_category, zc_arraylist_t * new_rules)
{
	...
	if (cdlog_category_obtain_rules(a_category, new_rules)) {
		zc_error("cdlog_category_obtain_rules fail");
		a_category->fit_rules = NULL;
		return -1;
	}
...
int cdlog_category_output(cdlog_category_t * a_category, cdlog_thread_t * a_thread)
{
	...
	zc_arraylist_foreach(a_category->fit_rules, i, a_rule) {
		rc = cdlog_rule_output(a_rule, a_thread);
	}
