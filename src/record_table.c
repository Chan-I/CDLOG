...
void cdlog_record_table_profile(zc_hashtable_t * records, int flag)
{
	...
	zc_hashtable_foreach(records, a_entry) {
		a_record = (cdlog_record_t *) a_entry->value;
		cdlog_record_profile(a_record, flag);
	}
	return;
}
...
void cdlog_record_table_del(zc_hashtable_t * records)
{
	...
	zc_debug("cdlog_record_table_del[%p]", records);
	return;
}
...
zc_hashtable_t *cdlog_record_table_new(void)
{
	...
	records = zc_hashtable_new(20,
			 ...
			 NULL, (zc_hashtable_del_fn) cdlog_record_del);
	if (!records) {
		zc_error("zc_hashtable_new fail");
		return NULL;
	} else {
		cdlog_record_table_profile(records, ZC_DEBUG);
		return records;
	}
}
/*******************************************************************************/
