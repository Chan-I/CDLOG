...
#define ZLOG_CONF_BACKUP_ROTATE_LOCK_FILE "/tmp/cdlog.lock"
/*******************************************************************************/
...
void cdlog_conf_profile(cdlog_conf_t * a_conf, int flag)
{
	...
	if (a_conf->default_format) {
		...
		cdlog_format_profile(a_conf->default_format, flag);
	}
	...
	if (a_conf->rotater) cdlog_rotater_profile(a_conf->rotater, flag);

	if (a_conf->levels) cdlog_level_list_profile(a_conf->levels, flag);

	if (a_conf->formats) {
		zc_profile(flag, "--format list[%p]--", a_conf->formats);
		zc_arraylist_foreach(a_conf->formats, i, a_format) {
			cdlog_format_profile(a_format, flag);
		}
	}
	...
	if (a_conf->rules) {
		...
		zc_arraylist_foreach(a_conf->rules, i, a_rule) {
			cdlog_rule_profile(a_rule, flag);
		}
	}
...
{
	zc_assert(a_conf,);
	if (a_conf->rotater) cdlog_rotater_del(a_conf->rotater);
	if (a_conf->levels) cdlog_level_list_del(a_conf->levels);
	if (a_conf->default_format) cdlog_format_del(a_conf->default_format);
	if (a_conf->formats) zc_arraylist_del(a_conf->formats);
	if (a_conf->rules) zc_arraylist_del(a_conf->rules);
	free(a_conf);
	zc_debug("cdlog_conf_del[%p]");
	return;
}

static int cdlog_conf_build_without_file(cdlog_conf_t * a_conf);
static int cdlog_conf_build_with_file(cdlog_conf_t * a_conf);

cdlog_conf_t *cdlog_conf_new(const char *confpath)
{
	int nwrite = 0;
	int has_conf_file = 0;
	cdlog_conf_t *a_conf = NULL;

	a_conf = calloc(1, sizeof(cdlog_conf_t));
	if (!a_conf) {
...
*/
	...
	a_conf->levels = cdlog_level_list_new();
	if (!a_conf->levels) {
		zc_error("cdlog_level_list_new fail");
		goto err;
	} 

	a_conf->formats = zc_arraylist_new((zc_arraylist_del_fn) cdlog_format_del);
	if (!a_conf->formats) {
		zc_error("zc_arraylist_new fail");
		goto err;
	}

	a_conf->rules = zc_arraylist_new((zc_arraylist_del_fn) cdlog_rule_del);
	if (!a_conf->rules) {
	...
	if (has_conf_file) {
		...
		if (cdlog_conf_build_without_file(a_conf)) {
			zc_error("cdlog_conf_build_without_file fail");
			goto err;
		}
	}

	cdlog_conf_profile(a_conf, ZC_DEBUG);
	return a_conf;
err:
	cdlog_conf_del(a_conf);
	return NULL;
}
/*******************************************************************************/
static int cdlog_conf_build_without_file(cdlog_conf_t * a_conf)
{
	...
	if (!a_conf->default_format) {
		zc_error("cdlog_format_new fail");
		return -1;
	}

	a_conf->rotater = cdlog_rotater_new(a_conf->rotate_lock_file);
	if (!a_conf->rotater) {
	...
	if (!default_rule) {
		zc_error("cdlog_rule_new fail");
		return -1;
	}

	/* add default rule */
	if (zc_arraylist_add(a_conf->rules, default_rule)) {
		cdlog_rule_del(default_rule);
...
/*******************************************************************************/
static int cdlog_conf_parse_line(cdlog_conf_t * a_conf, char *line, int *section);

static int cdlog_conf_build_with_file(cdlog_conf_t * a_conf)
{
	int rc = 0;
	struct cdlog_stat a_stat;
	...
	while (fgets((char *)pline, sizeof(line) - (pline - line), fp) != NULL) {
		...
		*++p = '\0';
		 ...
		 */
		rc = cdlog_conf_parse_line(a_conf, line, &section);
		if (rc < 0) {
...
static int cdlog_conf_parse_line(cdlog_conf_t * a_conf, char *line, int *section)
{
	...
	if (line[0] == '[') {
		...
		if (*section == 4) {
			...
			a_conf->default_format = cdlog_format_new(a_conf->default_format_line,
							&(a_conf->time_cache_count));
			if (!a_conf->default_format) {
				zc_error("cdlog_format_new fail");
				return -1;
	...
	case 1:
		...
		break;
	case 2:
		if (cdlog_level_list_set(a_conf->levels, line)) {
			zc_error("cdlog_level_list_set fail");
			if (a_conf->strict_init) return -1;
		}
		break;
	case 3:
		a_format = cdlog_format_new(line, &(a_conf->time_cache_count));
		if (!a_format) {
			zc_error("cdlog_format_new fail [%s]", line);
			if (a_conf->strict_init) return -1;
			else break;
		}
	...
	case 4:
		...
		if (!a_rule) {
			zc_error("cdlog_rule_new fail [%s]", line);
			if (a_conf->strict_init) return -1;
			else break;
		}
