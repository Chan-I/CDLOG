...
void cdlog_rotater_del(cdlog_rotater_t *a_rotater)
{
	...
	zc_debug("cdlog_rotater_del[%p]", a_rotater);
    free(a_rotater);
	return;
}
...
{
	int fd = 0;
	cdlog_rotater_t *a_rotater;

	zc_assert(lock_file, NULL);

	a_rotater = calloc(1, sizeof(cdlog_rotater_t));
	if (!a_rotater) {
	 ...
	 * if user A create /tmp/cdlog.lock 0600
	 * user B is unable to read /tmp/cdlog.lock
	 * B has to choose another lock file except /tmp/cdlog.lock
	 */
	...
	//cdlog_rotater_profile(a_rotater, ZC_DEBUG);
	return a_rotater;
err:
	cdlog_rotater_del(a_rotater);
	return NULL;
}

/*******************************************************************************/

static void cdlog_file_del(cdlog_file_t * a_file)
{
...
static cdlog_file_t *cdlog_file_check_new(cdlog_rotater_t * a_rotater, const char *path)
{
	int nwrite;
	int nread;
	cdlog_file_t *a_file;
	...
	a_file = calloc(1, sizeof(cdlog_file_t));
	if (!a_file) {
		zc_error("calloc fail, errno[%d]", errno);
		return NULL;
	}
...
static int cdlog_file_cmp(cdlog_file_t * a_file_1, cdlog_file_t * a_file_2)
{
	return (a_file_1->index > a_file_2->index);
}

static int cdlog_rotater_add_archive_files(cdlog_rotater_t * a_rotater)
{
	int rc = 0;
	glob_t glob_buf;
	size_t pathc;
	...
	a_rotater->files = zc_arraylist_new((zc_arraylist_del_fn)cdlog_file_del);
	if (!a_rotater->files) {
	...
	for (; pathc-- > 0; pathv++) {
		a_file = cdlog_file_check_new(a_rotater, *pathv);
		if (!a_file) {
		...
		rc = zc_arraylist_sortadd(a_rotater->files,
					(zc_arraylist_cmp_fn)cdlog_file_cmp, a_file);
		if (rc) {
...
static int cdlog_rotater_seq_files(cdlog_rotater_t * a_rotater)
{
	int rc = 0;
	int nwrite = 0;
	int i, j;
...
static int cdlog_rotater_roll_files(cdlog_rotater_t * a_rotater)
{
	int i;
	int rc = 0;
	int nwrite;
...
static int cdlog_rotater_parse_archive_path(cdlog_rotater_t * a_rotater)
{
	int nwrite;
	int nread;
	char *p;
...
static void cdlog_rotater_clean(cdlog_rotater_t *a_rotater)
{
...
static int cdlog_rotater_lsmv(cdlog_rotater_t *a_rotater, 
		char *base_path, char *archive_path, int archive_max_count)
{
	...
	rc = cdlog_rotater_parse_archive_path(a_rotater);
	if (rc) {
		zc_error("cdlog_rotater_parse_archive_path fail");
		goto err;
	}

	rc = cdlog_rotater_add_archive_files(a_rotater);
	if (rc) {
		zc_error("cdlog_rotater_add_archive_files fail");
		goto err;
	}

	if (a_rotater->mv_type == ROLLING) {
		rc = cdlog_rotater_roll_files(a_rotater);
		if (rc) {
			zc_error("cdlog_rotater_roll_files fail");
			goto err;
		}
	} else if (a_rotater->mv_type == SEQUENCE) {
		rc = cdlog_rotater_seq_files(a_rotater);
		if (rc) {
			zc_error("cdlog_rotater_seq_files fail");
			goto err;
		}
	}

	cdlog_rotater_clean(a_rotater);
	return 0;
err:
	cdlog_rotater_clean(a_rotater);
	return -1;
...
static int cdlog_rotater_trylock(cdlog_rotater_t *a_rotater)
{
	int rc;
	struct flock fl;
...
static int cdlog_rotater_unlock(cdlog_rotater_t *a_rotater)
{
	int rc = 0;
	struct flock fl;
		...
		char *archive_path, long archive_max_size, int archive_max_count)
{
	...
	if (cdlog_rotater_trylock(a_rotater)) {
		zc_warn("cdlog_rotater_trylock fail, maybe lock by other process or threads");
		return 0;
	}
	...
	rc = cdlog_rotater_lsmv(a_rotater, base_path, archive_path, archive_max_count);
	if (rc) {
		zc_error("cdlog_rotater_lsmv [%s] fail, return", base_path);
		rc = -1;
	} /* else if (rc == 0) */

	//zc_debug("cdlog_rotater_file_ls_mv success");

exit:
	/* unlock file */
	if (cdlog_rotater_unlock(a_rotater)) {
		zc_error("cdlog_rotater_unlock fail");
	}
