...
static int cdlog_rule_output_static_file_single(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	struct stat stb;
	...
	if (write(a_rule->static_fd,
			cdlog_buf_str(a_thread->msg_buf),
			cdlog_buf_len(a_thread->msg_buf)) < 0) {
...
static char * cdlog_rule_gen_archive_path(cdlog_rule_t *a_rule, cdlog_thread_t *a_thread)
{
	int i;
	cdlog_spec_t *a_spec;
	...
	cdlog_buf_restart(a_thread->archive_path_buf);

	zc_arraylist_foreach(a_rule->archive_specs, i, a_spec) {
		if (cdlog_spec_gen_archive_path(a_spec, a_thread)) {
			zc_error("cdlog_spec_gen_path fail");
			return NULL;
		}
	}

	cdlog_buf_seal(a_thread->archive_path_buf);
	return cdlog_buf_str(a_thread->archive_path_buf);
}

static int cdlog_rule_output_static_file_rotate(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	size_t len;
	struct cdlog_stat info;
	int fd;
	...
	len = cdlog_buf_len(a_thread->msg_buf);
	if (write(fd, cdlog_buf_str(a_thread->msg_buf), len) < 0) {
		zc_error("write fail, errno[%d]", errno);
		close(fd);
		return -1;
	}
	...
	if (cdlog_rotater_rotate(cdlog_env_conf->rotater, 
		a_rule->file_path, len,
		cdlog_rule_gen_archive_path(a_rule, a_thread),
		a_rule->archive_max_size, a_rule->archive_max_count)
		) {
		zc_error("cdlog_rotater_rotate fail");
		return -1;
 ...
 * return NULL	fail
 */
#define cdlog_rule_gen_path(a_rule, a_thread) do {    \
	int i;    \
	cdlog_spec_t *a_spec;    \
    \
	cdlog_buf_restart(a_thread->path_buf);    \
    \
	zc_arraylist_foreach(a_rule->dynamic_specs, i, a_spec) {    \
		if (cdlog_spec_gen_path(a_spec, a_thread)) {    \
			zc_error("cdlog_spec_gen_path fail");    \
			return -1;    \
		}    \
	}    \
    \
	cdlog_buf_seal(a_thread->path_buf);    \
} while(0)


static int cdlog_rule_output_dynamic_file_single(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	int fd;

	cdlog_rule_gen_path(a_rule, a_thread);

	if (cdlog_format_gen_msg(a_rule->format, a_thread)) {
		zc_error("cdlog_format_output fail");
		return -1;
	}

	fd = open(cdlog_buf_str(a_thread->path_buf),
		a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
		zc_error("open file[%s] fail, errno[%d]", cdlog_buf_str(a_thread->path_buf), errno);
		return -1;
	}

	if (write(fd, cdlog_buf_str(a_thread->msg_buf), cdlog_buf_len(a_thread->msg_buf)) < 0) {
		zc_error("write fail, errno[%d]", errno);
		close(fd);
		return -1;
	}
...
static int cdlog_rule_output_dynamic_file_rotate(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	int fd;
	char *path;
	size_t len;
	...
	path = cdlog_buf_str(a_thread->path_buf);
...
static int cdlog_rule_output_pipe(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	if (cdlog_format_gen_msg(a_rule->format, a_thread)) {
		zc_error("cdlog_format_gen_msg fail");
		return -1;
...
static int cdlog_rule_output_syslog(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	...
	cdlog_buf_seal(a_thread->msg_buf);
	syslog(a_rule->syslog_facility | a_level->syslog_level,
		"%s",  cdlog_buf_str(a_thread->msg_buf));
	return 0;
}

static int cdlog_rule_output_static_record(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	cdlog_msg_t msg;
	...
	msg.buf = cdlog_buf_str(a_thread->msg_buf);
	msg.len = cdlog_buf_len(a_thread->msg_buf);
...
static int cdlog_rule_output_dynamic_record(cdlog_rule_t * a_rule, cdlog_thread_t * a_thread)
{
	cdlog_msg_t msg;
	...
	msg.buf = cdlog_buf_str(a_thread->msg_buf);
	msg.len = cdlog_buf_len(a_thread->msg_buf);
	msg.path = cdlog_buf_str(a_thread->path_buf);
...
static int cdlog_rule_output_stdout(cdlog_rule_t * a_rule,
...
static int cdlog_rule_output_stderr(cdlog_rule_t * a_rule,
				   cdlog_thread_t * a_thread)
{
...
static int cdlog_rule_parse_path(char *path_start, /* start with a " */
		...
		int *time_cache_count)
{
	...
	specs = zc_arraylist_new((zc_arraylist_del_fn)cdlog_spec_del);
	if (!path_specs) {
...
err:
	...
	if (a_spec) cdlog_spec_del(a_spec);
	return -1;
}
		...
		int * time_cache_count)
{
	...
	a_rule = calloc(1, sizeof(cdlog_rule_t));
	if (!a_rule) {
	...
	a_rule->level = cdlog_level_list_atoi(levels, p);
	...
	if (STRCMP(format_name, ==, "")) {
		...
		int i;
		int find_flag = 0;
		cdlog_format_t *a_format;

		zc_arraylist_foreach(formats, i, a_format) {
			if (cdlog_format_has_name(a_format, format_name)) {
	...
	case '"' :
		...
		rc = cdlog_rule_parse_path(p, a_rule->file_path, sizeof(a_rule->file_path),
				&(a_rule->dynamic_specs), time_cache_count);
		if (rc) {
			zc_error("cdlog_rule_parse_path fail");
			goto err;
		}
		...
		if (a_rule->dynamic_specs) {
			if (a_rule->archive_max_size <= 0) {
				a_rule->output = cdlog_rule_output_dynamic_file_single;
			} else {
				a_rule->output = cdlog_rule_output_dynamic_file_rotate;
			}
		} else {
			struct stat stb;

			if (a_rule->archive_max_size <= 0) {
				a_rule->output = cdlog_rule_output_static_file_single;
			} else {
				/* as rotate, so need to reopen everytime */
				a_rule->output = cdlog_rule_output_static_file_rotate;
			}
	...
	case '|' :
		...
		a_rule->output = cdlog_rule_output_pipe;
		break;
	case '>' :
	...
	case '$' :
		...
		if (strchr(a_rule->record_path, '%') == NULL) {
			a_rule->output = cdlog_rule_output_static_record;
		} else {
			cdlog_spec_t *a_spec;

			a_rule->output = cdlog_rule_output_dynamic_record;

			a_rule->dynamic_specs = zc_arraylist_new((zc_arraylist_del_fn)cdlog_spec_del);
			if (!(a_rule->dynamic_specs)) {
				zc_error("zc_arraylist_new fail");
				goto err;
			}
...
void cdlog_rule_del(cdlog_rule_t * a_rule)
{
	...
	zc_debug("cdlog_rule_del[%p]", a_rule);
    free(a_rule);
	return;
}
...
int cdlog_rule_set_record(cdlog_rule_t * a_rule, zc_hashtable_t *records)
{
	...
	if (a_rule->output != cdlog_rule_output_static_record 
	&&  a_rule->output != cdlog_rule_output_dynamic_record) {
