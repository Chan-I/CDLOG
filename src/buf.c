 ...
 * So, what should you do if you want to using cdlog on the platform that doesn't
 * conform C99? My Answer is, crack cdlog with a portable C99-vsnprintf, like this
...
void cdlog_buf_del(cdlog_buf_t * a_buf)
{
	...
	zc_debug("cdlog_buf_del[%p]", a_buf);
    free(a_buf);
	return;
}

cdlog_buf_t *cdlog_buf_new(size_t buf_size_min, size_t buf_size_max, const char *truncate_str)
{
	cdlog_buf_t *a_buf;
	...
	//cdlog_buf_profile(a_buf, ZC_DEBUG);
	return a_buf;
...
/*******************************************************************************/
static void cdlog_buf_truncate(cdlog_buf_t * a_buf)
{
	char *p;
	size_t len;
 ...
 * increment must > 0
 */
static int cdlog_buf_resize(cdlog_buf_t * a_buf, size_t increment)
{
	int rc = 0;
	size_t new_size = 0;
	size_t len = 0;
...
int cdlog_buf_vprintf(cdlog_buf_t * a_buf, const char *format, va_list args)
{
	...
	if (!a_buf->start) {
		zc_error("pre-use of cdlog_buf_resize fail, so can't convert");
		return -1;
	}
	...
	if (nwrite >= 0 && nwrite < size_left) {
		...
		rc = cdlog_buf_resize(a_buf, nwrite - size_left + 1);
		if (rc > 0) {
			...
			//*(a_buf->tail) = '\0';
			cdlog_buf_truncate(a_buf);
			return 1;
		} else if (rc < 0) {
			zc_error("cdlog_buf_resize fail");
			return -1;
		} else {
			//zc_debug("cdlog_buf_resize succ, to[%ld]", a_buf->size_real);
...
int cdlog_buf_printf_dec32(cdlog_buf_t * a_buf, uint32_t ui32, int width)
{
	...
	if ((q = a_buf->tail + out_len) > a_buf->end) {
		...
		rc = cdlog_buf_resize(a_buf, out_len - (a_buf->end - a_buf->tail));
		if (rc > 0) {
			size_t len_left;
...
int cdlog_buf_append(cdlog_buf_t * a_buf, const char *str, size_t str_len)
{
	...
	if ((p = a_buf->tail + str_len) > a_buf->end) {
		int rc;
		//zc_debug("size_left not enough, resize");
		rc = cdlog_buf_resize(a_buf, str_len - (a_buf->end - a_buf->tail));
		if (rc > 0) {
			size_t len_left;
		...
		int left_adjust, int zero_pad, size_t in_width, size_t out_width)
{
	...
	if (append_len > a_buf->end - a_buf->tail) {
		int rc = 0;
		//zc_debug("size_left not enough, resize");
		rc = cdlog_buf_resize(a_buf, append_len - (a_buf->end -a_buf->tail));
		if (rc > 0) {
			zc_error("conf limit to %ld, can't extend, so output", a_buf->size_max);
