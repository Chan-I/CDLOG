...
void cdlog_event_del(cdlog_event_t * a_event)
{
	...
	zc_debug("cdlog_event_del[%p]", a_event);
    free(a_event);
	return;
}
...
{
	cdlog_event_t *a_event;

	a_event = calloc(1, sizeof(cdlog_event_t));
	if (!a_event) {
		zc_error("calloc fail, errno[%d]", errno);
		return NULL;
	}

	a_event->time_caches = calloc(time_cache_count, sizeof(cdlog_time_cache_t));
	...
	/*
	 * at the cdlog_init we gethostname,
	...
	//cdlog_event_profile(a_event, ZC_DEBUG);
	return a_event;
err:
	cdlog_event_del(a_event);
	return NULL;
}
...
{
	/*
	 * category_name point to cdlog_category_output's category.name
	 */
	 ...
	 * cdlog_spec_write_time gettimeofday
	 */
